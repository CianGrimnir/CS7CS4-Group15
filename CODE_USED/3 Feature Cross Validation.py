...
cross-validating various three-feature regression models.
"""
...
def CrossVal(filename1, filename2, filename3):
    ...
    for i in range(len(feature1[years[0]])):
        ...
        X3_temp = feature3.iloc[i,:]
        y_temp = target.iloc[i,:]
        
        for j in years:
            if X1_temp[j] != '#' and X2_temp[j] != '#' and X3_temp[j] != '#':
                X.append([float(X1_temp[j]),float(X2_temp[j]),float(X3_temp[j])])
                y.append(y_temp[j])
    ...
    report3 = filename3.split('_')
    
    filename = report1[0]+"&"+report2[0]+"&"+report3[0]+"_"+str(min(years))+"-"+str(max(years))
    
    PrintSet(filename, "_train", X_train, y_train)
    PrintSet(filename, "_test", X_test, y_test)
    PrintSet(filename, "_val", X_val, y_val)
    
    reportname = report1[0]+"&"+report2[0]+"&"+report3[0]
...
CrossVal("MaleLifeExpect_2000-2017","ChildPov18_2006-2016","NEETs_2009-2015")
CrossVal("Traffic_2000-2017","Female65LifeExpect_2000-2017","Earnings_2000-2017")